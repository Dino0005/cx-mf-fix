...
struct ContentView: View {
    ...
    private var tabBar: some View {
        HStack(spacing: 0) {
            tabButton(title: "MF Fix", icon: "waveform.path.ecg", index: 0)
            tabButton(title: "GStreamer", icon: "play.rectangle.fill", index: 1)
        }
    ...
    private func tabButton(title: String, icon: String, index: Int) -> some View {
        Button(action: { selectedTab = index }) {
            ...
            .foregroundColor(selectedTab == index ? .white : .gray.opacity(0.5))
            .frame(maxWidth: .infinity)
            .padding(.vertical, 14)
            .background(
                VStack {
...
// MARK: - MFFixView (ex ContentView)

struct MFFixView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !fixCompleted {
                ...
                .onTapGesture { openBottlesFolder() }
            }
            ...
            HStack(spacing: 20) {
                ...
                if selectedPath != nil && !isProcessing && !fixCompleted {
                    Button(action: { showPopupWarning() }) {
        ...
        .alert(L10n.alertWarningTitle, isPresented: $showingPopupWarning) {
            ...
            Button(L10n.alertButtonContinue) { applyFix() }
        } message: {
        ...
        .alert(resultIsSuccess ? L10n.alertSuccessTitle : L10n.alertErrorTitle,
            ...
            Button(L10n.alertButtonOK, role: .cancel) {
                if resultIsSuccess { fixCompleted = true }
            }
        } message: {
            Text(resultMessage)
        }
    ...
    private func showPopupWarning() { showingPopupWarning = true }
    ...
    private func handleDrop(providers: [NSItemProvider]) {
        ...
        provider.loadItem(forTypeIdentifier: UTType.fileURL.identifier, options: nil) { item, _ in
            ...
            DispatchQueue.main.async { validateAndSetPath(url.path) }
        }
    }

    private func validateAndSetPath(_ path: String) {
        if FileManager.default.fileExists(atPath: path + "/drive_c") {
            selectedPath = path
        } else {
    ...
    private func applyFix() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                resultMessage = success ? L10n.alertSuccessMessage : L10n.alertErrorMessage
                showingResult = true
            }
        }
    }
